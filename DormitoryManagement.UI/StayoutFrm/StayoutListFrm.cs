...
namespace DormitoryManagement.UI.StayoutFrm
{
    ...
    /// 退宿扣款窗体
    /// </summary>
    public partial class StayoutListFrm : Form
    {
        private StayoutBll bll = new StayoutBll();

        /// <summary>
        /// 页面初始化加载窗体
        /// </summary>
        public StayoutListFrm()
        {
            InitializeComponent();
        }
        ...
        /// <param name="e"></param>
        private void StayoutListFrm_Load(object sender, EventArgs e)
        {
            GetStaffStaffStayOutDto();
        ...
        /// 从数据库中获取数据
        /// </summary>
        public void GetStaffStaffStayOutDto()
        {
            var QSTime = dpQSTime.Value.ToString("yyyy-MM-dd");
            var ZZTimme = dpZZTime.Value.ToString("yyyy-MM-dd");
            PageResultDto<StaffStaffStayOutDto> list = bll.GetStaffStaffStayOutDto(QSTime, ZZTimme, pageIndex, pageSize);
        ...
        public List<StaffStaffStayOutDto> GetExportData()
        {
            ...
            var list = bll.GetStaffStaffStayOutList(QSTime, ZZTime);
            return list;
        }
        ...
        private void butExport_Click(object sender, EventArgs e)
        {
            ...
            dataRow.CreateCell(10).SetCellStyle(workbook, "扣款");
            var row = 1;
            list.ForEach(m =>
            {
